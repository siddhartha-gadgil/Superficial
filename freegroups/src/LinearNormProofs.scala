...
object LinearNormProofs {
  ...
  def update(pf: LinNormBound, noSym: Boolean) =
    memoNormProof.get(pf.word).foreach { (mpf) =>
      if (mpf.bound > pf.bound) save(pf.word, pf, noSym)
    }
  ...
  def proofPowers(
      pf: LinNormBound,
      n: Int,
      accum: Vector[LinNormBound] = Vector()): Vector[LinNormBound] =
  ...
  def updateInverses() =
    ...
    } if (invPf.bound < pf.bound)
      memoNormProof += (w -> LinNormBound.inverse(invPf))
        ...
        .filter {
          case (w, p) =>
            (memoNormProof
              .get(w ++ pf.word)
              .map(_.bound)
              .getOrElse(0.0) > (pf.bound + p.bound))
        }
  ...
  def fc(n: Int = 10, noSym: Boolean = true) =
    scaledTaskProofs(c, 1, n, noSym).runAsync
  def fcna(k: Int, n: Int = 10, noSym: Boolean = true) =
    scaledTaskProofs(cna(k), 1, n, noSym).runAsync
  def fcnab(k: Int, n: Int = 10, noSym: Boolean = true) =
    scaledTaskProofs(cnab(k), 1, n, noSym).runAsync
  def fcnB(k: Int, n: Int = 10, noSym: Boolean = true) =
    scaledTaskProofs(cnB(k), 1, n, noSym).runAsync
